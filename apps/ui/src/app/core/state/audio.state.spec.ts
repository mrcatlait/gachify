...
import { PlayerStatus, Remix } from '@core/models'
...
describe('AudioState', () => {
  ...
  it('should load the remix and set the initial state', () => {
    // Arrange
    const remix = { id: '1', duration: 120 } as Remix

    // Act
    audioState.load({ remix })

    // Assert
    expect(audioState.status()).toBe(PlayerStatus.Loading)
    expect(audioState.duration()).toBe(remix.duration)
    expect(audioState.currentTime()).toBe(0)
    expect(audioServiceMock.load).toHaveBeenCalledWith(remix.id)
