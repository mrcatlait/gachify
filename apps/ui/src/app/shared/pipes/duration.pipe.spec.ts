...
describe('DurationPipe', () => {
  ...
  it('should transform duration of 3660 to "61:00"', () => {
    const duration = 3660
    const transformedDuration = pipe.transform(duration)
    expect(transformedDuration).toEqual('61:00')
  })

  it('should return an empty string for undefined duration', () => {
    const duration = undefined
    const transformedDuration = pipe.transform(duration)
    expect(transformedDuration).toEqual('')
  })

  it('should return an empty string for null duration', () => {
    const duration = null
    const transformedDuration = pipe.transform(duration)
    expect(transformedDuration).toEqual('')
  })
